  ...
  const post = async () => {
    ...
    if (
      ...
      if(bannerMedia){
        ...
        formData.append('media', bannerMediaPreview)
      }

      if(bannerBackground){
        formData.append('file[]', bannerBackground)
      } else {
        formData.append('background', bannerBackgroundPreview)
      }
  ...
  const getBannerDetailsHandler = async (id) => {
    ...
    try {
      ...
      if (res.data.statusCode === 200) {
        ...
        setbannerBackgroundType(res.data.result?.backgroundType ? res.data.result?.mediaType : '')
